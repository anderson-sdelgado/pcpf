...
main() {
  ...
  final deleteAllLocal = DeleteAllLocalImpl(localRepository);

  group('Test DeleteAllLocal', () {

    test('deve retornar true se apagar os dados', () async {
      when(localRepository.deleteAllLocal()).thenAnswer((_) => Future(() => const Right(true)));
      final result = await deleteAllLocal();
      expect(result.fold(id, id), true);
    });

    test('deve retornar false se tiver erro apagar os dados', () async {
      when(localRepository.deleteAllLocal()).thenAnswer((_) => Future(() => Left(ErrorFloorDatasource("Erro"))));
      final result = await deleteAllLocal();
